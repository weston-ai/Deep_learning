...
### OPTIONAL: Identify the optimal number of workers (logical cores) to be used in DataLoader
from py_utils import benchmark_dataloader # import benchmark_dataloader from my utils
...
# ==============================
# 📊 Benchmarking Train Dataset
# ==============================
# Dataset size: 28886 samples
#   ➤ Testing DataLoader with num_workers = 0
#      ⏱️  Time taken: 0.1400 seconds
#   ➤ Testing DataLoader with num_workers = 2
#      ⏱️  Time taken: 0.7593 seconds
#   ➤ Testing DataLoader with num_workers = 4
#      ⏱️  Time taken: 0.6548 seconds
#   ➤ Testing DataLoader with num_workers = 6
#      ⏱️  Time taken: 0.6344 seconds
#   ➤ Testing DataLoader with num_workers = 8
#      ⏱️  Time taken: 0.6407 seconds

##### Alternative method for creating tensors from preprocessed data (NOT SUPPORTED BY DATALOADER)
# def to_tensor(X, y):
#     # Reset index defensively (this resets the index to 0, 1, 2, 3, etc, if it detects pd.Series type, which is what was created in the test_train_split)
#     if isinstance(y, pd.Series):
#         y = y.reset_index(drop=True)
...
#     X_tensor = torch.tensor(X.toarray(), dtype=torch.float32) # toarray() converts it to dense matrix
#     y_tensor = torch.tensor(y.astype(np.float32)).unsqueeze(1)
#     return X_tensor, y_tensor
#
# X_train_tensor, y_train_tensor = to_tensor(X_train_transformed, y_train)
# X_valid_tensor, y_valid_tensor = to_tensor(X_valid_transformed, y_valid)
# X_test_tensor, y_test_tensor = to_tensor(X_test_transformed, y_test)

###### Laziest method for creating tensors (ALSO NOT SUPPORTED BY DATALOADER)
# # Training
# X_train_tensor = torch.tensor(X_train_transformed.toarray(), dtype=torch.float32)
# y_train_tensor = torch.tensor(y_train.values.astype(np.float32)).unsqueeze(1) # unsqueeze(1) is a good choice for binary classification or regression with a single output neuron
# # Validation
# X_valid_tensor = torch.tensor(X_valid_transformed.toarray(), dtype=torch.float32)
# y_valid_tensor = torch.tensor(y_valid.values.astype(np.float32)).unsqueeze(1)
# # Testing
...
def evaluate_best_model_on_test_data(test_dataset, input_size, json_best_auc_path, best_model_weights_path, plot_output_dir, table_output_dir):
    ...
    #
    # # Function to reconstruct the model with an assigned input architecture
    # def reconstruct_model(input_size, metadata):
    #     return DynamicBinaryClassifier(
    #         input_size=input_size,
    #         hidden_size=int(metadata["hidden_size"]),
...
<h2>📌 Objective</h2>
<p>This project developed a deep feedforward binary classifier in PyTorch to predict whether a patient developed hypertension within a 24-month observation window. The total dataset contained 60,000 simulated human patients that were matched with appropriate measures for individual and community confounding. The pre-training split used class-stratified sampling to assign 37,500 patients to the training dataset; 11,250 patients to the validation dataset; and 11,250 patients to the testing dataset. Each input vector is cross-sectional, with no temporal sequence. </p>
...
<ul>
    ...
    <li>Output layer: raw logits (BCEWithLogitsLoss used)</li>
</ul>
...
<p>Two models were retained:</p>
<ul>
    <li>🌟 Best Validation Loss Model</li>
    <li>🌟 Best ROC AUC Model</li>
</ul>

<hr>

<h2>📊 Training Curves — Best Validation Loss Model</h2>
"""
    display_names = {
        "loss": "Loss",
        "accuracy": "Accuracy",
    ...
    for metric in metrics:
        relative_best_plot_path = f"plots/EverHadHT/best_training_models/{model_id}_{metric}_best_val_loss_{timestamp}.png"
        html += f"""
...
"""
    ...
    html += """
<hr>
<h2>🧪 Test Evaluation — Best ROC AUC Model</h2>
<p>Final performance of the optimized neural net model was evaluated on a held-out test dataset (n=20,000 observations). Below are the evaluation curves:</p>
"""
    ...
    html += f"""
<h3>{display_names.get("roc_auc", "ROC Curve")}</h3>
<img src="{roc_curve_path}" alt="ROC Curve">

<h3>{display_names.get("pr_auc", "Precision-Recall Curve")}</h3>
...
"""
    ...
    <h3>ROC Curve Comparison (Neural Net vs Logistic Regression)</h3>
    <img src="{roc_curve_path}" alt="NN ROC Curve">
    <img src="{log_reg_results['roc_plot_path']}" alt="LogReg ROC Curve">

    <h3>Precision-Recall Curve Comparison (Neural Net vs Logistic Regression)</h3>
...
report_path = os.path.join(base_output_dir, "hypertension_report.html")
...
html_path = os.path.join(base_output_dir, "hypertension_report.html")
pdf_path = os.path.join(base_output_dir, "hypertension_report.pdf")
...
with open("training_report.html", "w") as f:
    html_rendered = html_template.replace("{{ stop1 }}", str(stop_epoch1)).replace("{{ stop2 }}", str(stop_epoch2))
    f.write(html_rendered)

# Convert to PDF
HTML("training_report.html").write_pdf("training_report.pdf")
print("Report saved as training_report.pdf")

# Save model weights
os.chdir("/HT_RNN_BinCLass_EverHadHT_torch/model_weights")
torch.save(model1.state_dict(), "model_weights_HT_RNN_BinClass_EverHadHT_wo_ElasticNet.pth")
torch.save(model2.state_dict(), "model_weights_HT_RNN_BinClass_EverHadHT_w_ElasticNet.pth")